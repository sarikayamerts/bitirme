multinomial_model_with_z <- function(data){
  df$winner_category <- sapply(as.character(df$winner), switch, "odd1" = 1, "oddX" = 2, "odd2" = 3, USE.NAMES = F)
  df <- subset(df, select = c('matchId', 'bookmaker', 'z', 'shin_prob.odd1', 'shin_prob.oddX', 'shin_prob.odd2', 'winner_category'))
  ...
  mult <- multinom(winner_category ~ z + shin_prob.odd1 + shin_prob.oddX + shin_prob.odd2 , data = train, model = TRUE)
  summary(mult)
